...
%define STAGE2_SECTORS 8          ; Number of sectors for stage 2
...
load_stage2:
    ...
    ; Check for stage 2 signature at end of first sector
    mov ax, [es:510]
    cmp ax, 0xAA55
    jne disk_error
...
.loop:
    ...
    cmp cx, BUFFER_SIZE-1 ; Buffer full?
    je .loop
...
help_text:     db 'Available commands:', NEWLINE
               ...
               db '  reboot - Reboot the system', NEWLINE, 0
...
; Purpose: Enable A20 line using keyboard controller
;-----------------------------------------------------------------------------
enable_a20_kbc:
    pushf
    push ax
...
; Purpose: Wait for keyboard controller to be ready
;-----------------------------------------------------------------------------
kbc_wait_ready:
    push ax
.wait:
